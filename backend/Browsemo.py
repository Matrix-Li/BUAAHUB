...
def browsemo(SID,MOID):
    try:
        ...
        mod = dataforum.query(MODULE).filter(MODULE.MOID==MOID).first()

        allforum = dataforum.query(FORUM).filter(FORUM.MOID==MOID).all()

        Len = len(allforum)

        if Len==0:
            dataforum.close()
            return jsonwrap(4, "nothing.", res)
        
        
        for i in range(Len):
            tempfroum = allforum[i]
            thiscustomer = dataforum.query(CUSTOMER).filter(CUSTOMER.CID==tempfroum.CID).first()
            if(thiscustomer==None):
